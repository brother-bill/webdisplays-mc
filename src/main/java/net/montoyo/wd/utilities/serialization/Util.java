...
public abstract class Util {
    ...
    /**
     * Write an ItemStack to a FriendlyByteBuf using item registry name and count.
     * This approach works without needing RegistryFriendlyByteBuf.
     */
    public static void writeItemStack(FriendlyByteBuf buf, net.minecraft.world.item.ItemStack stack) {
        if (stack.isEmpty()) {
            buf.writeBoolean(false);
        } else {
            buf.writeBoolean(true);
            buf.writeUtf(net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(stack.getItem()).toString());
            ...
            buf.writeNbt(customData.isEmpty() ? null : customData);
        }
    }

    /**
     * Read an ItemStack from a FriendlyByteBuf written by writeItemStack.
     */
    public static net.minecraft.world.item.ItemStack readItemStack(FriendlyByteBuf buf) {
        if (!buf.readBoolean()) {
